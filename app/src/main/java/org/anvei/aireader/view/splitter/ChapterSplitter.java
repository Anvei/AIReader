package org.anvei.aireader.view.splitter;

import androidx.annotation.IntRange;

import org.anvei.aireader.bean.ChapterSplitResult;
...
public class ChapterSplitter {
    ...
    private final List<String> chapterTitleRegexes = new ArrayList<>();       // 章节名匹配正则表达式
    ...
    public String getRegex(int position) {
        return chapterTitleRegexes.get(position);
    }

    public int getRegexCount() {
        return chapterTitleRegexes.size();
    }

    public void removeRegex(int position) {
        chapterTitleRegexes.remove(position);
    }

    public void addRegex(String regex) {
        chapterTitleRegexes.add(regex);
    }

    // 匹配全部正则表达式
    private boolean matchChapterTitle(String line, int index) {
        boolean res = true;
        for (String regex : chapterTitleRegexes) {
    ...
    // TODO: 处理文件编码问题
    public void startSplit() throws FileEncodingException, IOException {

        // 初始化
        randomAccessFile.seek(0);
        chapterSplitResults.clear();
        if (chapterTitleRegexes.isEmpty()) {
            chapterTitleRegexes.add(defaultChapterTittleRegex);
        }
    ...
    public int getChapterSplitResultCount() {
        return chapterSplitResults.size();
    }
